...
{
  if (this._debug) 
    console.error("Creating pg adapter");  
  this._dbClient = null;
  /**
   * @type boolean
   */
  ...
  if (connectionParams["dsn"]) {
    ...
    this._pgUrl = 'pg://' + cfg.user + ':' + cfg.password + '@' + cfg.host + (cfg.port ? (':' + cfg.port) : '') + '/' + cfg.database;
  }
...
Adapter.prototype.connect = function()
{
  ...
  if( this._debug )
    console.error('Adapter.prototype.connect('+JSON.stringify(this._connectionParams)+')');
...
Adapter.prototype.close = function(callback)
{
  ...
  if( this._debug )
    console.error('Adapter.prototype.close('+JSON.stringify(this._connectionParams)+')');
...
Adapter.prototype.query = function( sql, bind, callback )
{
  ...
  async.waterfall(
    ...
    function( err, res )
    {
      //if (err) {
      if (this._debug) {
        console.error('O: ' + origSql);
        console.error('S: ' + sql);
        console.error('E: ' + JSON.stringify(err));
        console.error('R: ' + JSON.stringify(res));
      }
