 ...
 * @api public
 */
DBWrapper.prototype._fetchRow = function(sql, ...args) {
    let [bind, callback] = unpackSelectArgs(args)
  
    async.waterfall([
        callback => this._fetchAll( sql, bind, callback )
    ], function (err, res) {
        if( err )
          callback( err );
 ...
 * @api public
 */
DBWrapper.prototype._fetchCol = function(sql, ...args) {
    let [bind, callback] = unpackSelectArgs(args)
 ...
 * @api public
 */
DBWrapper.prototype._fetchOne = function(sql, ...args) {
    let [bind, callback] = unpackSelectArgs(args)
  
    async.waterfall([
        callback => this._fetchRow(sql, bind, callback),
    ], (err, res) => {
        if (err)
            callback(err)
        else if( ! res )
